...
import { sportCategories, formatPrice } from "../data/products";
...
export function Home() {
  ...
  useEffect(() => {
    productService.getAllProducts().then(data => {
      // For now we just slice or filter based on what we have
          ...
          {blogPosts.slice(0, 3).map((post) => (
            <Link key={post.id} to={`/blog/${post.id}`} className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg hover:border-blue-100 transition-all">
              <div className="h-44 overflow-hidden relative">
                ...
                />
                <span className="absolute top-3 left-3 bg-white/95 text-blue-700 text-xs px-2.5 py-1 rounded-full">
                  {post.category}
                </span>
              </div>
              <div className="p-4">
                <h3 className="text-gray-900 text-sm leading-snug group-hover:text-blue-700 transition-colors line-clamp-2 mb-2">
                  {post.title}
                </h3>
